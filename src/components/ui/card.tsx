...
// Card del sistema v1: hairline, radio 12, relleno 20 px, SIN sombra.
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-5 rounded-card border py-5",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-h4 leading-snug font-semibold", className)}
      {...props}
    />
  )
}
